...
Designed for one NVIDIA RTX 4070 Ti SUPER with 16 GB VRAM and CUDA. Physical batch 2 with 48 accumulation steps reproduces effective batch 96. BF16 or FP16 and gradient checkpointing are used for memory. See [recovered setup](docs/original_experiment.md) for exact settings, uncertainties, and deviations.
...
On the RTX 4070 Ti SUPER, the initial two-step smoke run completed with **6,864 MiB peak reserved VRAM** using BF16, physical batch 2, and accumulation 48. This confirms the model fits for the tested batch size; it does not establish full-run accuracy.
